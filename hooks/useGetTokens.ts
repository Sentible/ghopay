...
export const useFetchTokenMetadata = (address?: string, id?: number) =>
  swr(address && id ? address : null, async () => await tokenFetcher(address!, id!), {
...
const useFetchTokenByNetwork = (url = '', address: string) =>
  swr(url || null, async () => await tokenBalanceFetcher(url!, address!), {
    revalidateOnFocus: false,
    revalidateOnMount: true,
  })
...
export const useGetTokens = () => {
  ...
  const goerliData = (useFetchTokenByNetwork(goerliDefaultRpcUrl, address)?.data?.result?.tokenBalances ??
    []) as AlchemyToken[]
  const optimismData = (useFetchTokenByNetwork(optimismDefaultRpcUrl, address)?.data?.result?.tokenBalances ??
    []) as AlchemyToken[]
  const polygonData = (useFetchTokenByNetwork(polygonDefaultRpcUrl, address)?.data?.result?.tokenBalances ??
    []) as AlchemyToken[]
  const mainnetData = (useFetchTokenByNetwork(mainnetDefaultRpcUrl, address)?.data?.result?.tokenBalances ??
    []) as AlchemyToken[]
  const arbitrumData = (useFetchTokenByNetwork(arbitrumDefaultRpcUrl, address)?.data?.result?.tokenBalances ??
    []) as AlchemyToken[]
  ...
  const {
    ...
    () =>
      ...
      } as Record<string, AlchemyToken[]>),
    [goerliData, optimismData, polygonData, mainnetData, arbitrumData],
  )
